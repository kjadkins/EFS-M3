...
//
// Description 
// This API is used to find an agent allocation in EXTAGA
// Transaction GetAgentAlloc
// 
...
public class GetAgentAlloc extends ExtendM3Transaction {
  ...
  // Constructor 
  public GetAgentAlloc(MIAPI mi, DatabaseAPI database, ProgramAPI program, LoggerAPI logger, MICallerAPI miCaller) {
     this.mi = mi;
  ...
  public void main() { 
     ...
     if (inPYNO != null && inPYNO != "" && inPYNO != "x") {
     ...
     if (inCUNO != null && inCUNO != "" && inCUNO != "x") {
     ...
     if (inCSCD != null && inCSCD != "" && inCSCD != "x") {
     ...
     if (inECAR != null && inECAR != "" && inECAR != "x") {
     ...
     if (inATAV != null && inATAV != "" && inATAV != "x") {
     ...
     if (inHIE3 != null && inHIE3 != "" && inHIE3 != "x") {
     ...
     Optional<DBContainer> EXTAGA = findEXTAGA(CONO, inPRIO, inPYNO, inCUNO, inCSCD, inECAR, inPONO, inATAV, inHIE3, inDATE)
     if(!EXTAGA.isPresent()){
        mi.error("Agent doesn't exists")   
        return             
     } else {
        DBContainer containerEXTAGA = EXTAGA.get() 
        outAGN1 = containerEXTAGA.getString("EXAGN1")
        outAGN2 = containerEXTAGA.getString("EXAGN2")
     }
