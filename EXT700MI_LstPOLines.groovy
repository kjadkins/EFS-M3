  ...
  //********************************************************************  
   void lstRecord(){   
     ...
     ExpressionFactory expression = database.getExpressionFactory("MPLINE")
	   ...
	   // Only get invoice lines in status 75-80
     if (regDate != 0) {
        expression = expression.lt("IBPUST", "81").and(expression.gt("IBPUST", "74")).and(expression.eq("IBRGDT", String.valueOf(regDate)))    
     } else if (regDate == 0) {
        expression = expression.lt("IBPUST", "81").and(expression.gt("IBPUST", "74"))                                             
     } else {
        expression = expression.le("IBPUST", "80").and(expression.ge("IBPUST", "75"))   
     }
